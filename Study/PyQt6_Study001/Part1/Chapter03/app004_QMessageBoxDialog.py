...
class MainWindow(QWidget):
    ...
    def searchAuthors(self):
        """Search through a catalogue of names. If a name is found, display the Author Found dialog. Otherwise, display Author Not Found dialog."""
        ...
        try:
            ...
            if self.author_edit.text() in authors:
                QMessageBox.information(self, "Author Found", "Author found in caltalogue!", QMessageBox.StandardButton.Ok)
            else:
                answer = QMessageBox.question(self, "Author Not Found", """<p>Author not found in catalogue.</p><p>Do you wish to continue?</p>""", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)

                if answer == QMessageBox.StandardButton.No:
                    print("Closing application.")
                    self.close()
        except FileNotFoundError as error:
            QMessageBox.warning(self, "Error", f"""<p>File not found.</p><p>{error}</p>""", QMessageBox.StandardButton.Ok)
            self.close()
